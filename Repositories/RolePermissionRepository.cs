...
public class RolePermissionRepository : BaseRepository<RolePermission>, IRolePermissionRepository
{
        ...
        DateTime? expiresAt = null)
    {
        ...
        // Role 정보 조회 (OrganizationId 가져오기 위함)
        ...
        string? reason = null)
    {
        ...
        await DeleteAsync(rolePermission.Id);
        return true;
    }
        ...
        Guid grantedBy)
    {
        ...
        // 기존 권한 모두 제거
        var removedCount = await RemoveAllPermissionsAsync(roleId);
        ...
        Guid grantedBy)
    {
        ...
        var targetRole = await _context.Set<Role>().FirstOrDefaultAsync(r => r.Id == targetRoleId);
        if (targetRole == null)
        {
        ...
        return await AddAsync(inheritedPermission);
    }
    ...
    {
        // 구현 복잡도를 고려하여 기본 로직만 제공
        // 실제 환경에서는 더 정교한 동기화 로직이 필요할 수 있음
    ...
    {
        return await Query()
            .Where(rp => rp.OrganizationId == organizationId && rp.IsActive)
    ...
    public async Task<IEnumerable<RolePermission>> SearchByScopePatternAsync(
        ...
        Guid organizationId)
    {
        // 와일드카드 지원을 위한 LIKE 패턴 변환
        var likePattern = scopePattern.Replace("*", "%");

        return await Query()
            .Where(rp => rp.OrganizationId == organizationId &&
                        EF.Functions.Like(rp.PermissionScope, likePattern))
            .Include(rp => rp.Role)
            .Include(rp => rp.Permission)
    ...
    public async Task<int> ReorderPrioritiesAsync(Guid roleId, IEnumerable<Guid> orderedPermissionIds)
    {
        ...
        if (updatedCount > 0)
        {
            await UpdateRangeAsync(rolePermissions.Where(rp => permissionIdsList.Contains(rp.PermissionId)));
        }
