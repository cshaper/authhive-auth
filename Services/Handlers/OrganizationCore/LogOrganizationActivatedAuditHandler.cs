// File: AuthHive.Auth/Services/Handlers/OrganizationCore/LogOrganizationActivatedAuditHandler.cs
...
{
    /// <summary>
    /// 조직 활성화 이벤트(OrganizationActivatedEvent) 발생 시 감사 로그를 기록합니다.
    /// </summary>
    public class LogOrganizationActivatedAuditHandler :
        IDomainEventHandler<OrganizationActivatedEvent>,
        IService
    {
        private readonly IAuditService _auditService;
        private readonly ILogger<LogOrganizationActivatedAuditHandler> _logger;

        public int Priority => 10; // 1순위: 감사 로그 핸들러
        public bool IsEnabled => true;

        public LogOrganizationActivatedAuditHandler(
            IAuditService auditService,
            ILogger<LogOrganizationActivatedAuditHandler> logger)
        {
            _auditService = auditService ?? throw new ArgumentNullException(nameof(auditService));
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        }

        public async Task HandleAsync(OrganizationActivatedEvent @event, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                const string action = "ORGANIZATION_ACTIVATED";
                // ❗️ 활성화는 긍정적인 상태 변경이므로 Info 등급
                var severity = AuditEventSeverity.Info; 

                _logger.LogInformation( // ❗️ Info 수준으로 로깅
                ...
                var auditData = new Dictionary<string, object>
                {
                    ...
                    ["reason"] = @event.Reason,
                    ["activated_by_connected_id"] = initiator ?? Guid.Empty,
            ...
            {
                _logger.LogError(ex, "Failed to log audit for OrganizationActivatedEvent: {EventId}", @event.EventId);
            }
        }
