...
{
    /// <summary>
    /// <see cref="UserAccountActivatedEvent"/>를 처리하는 감사 로그 핸들러입니다.
    /// </summary>
    public class LogUserAccountActivatedAuditHandler
        : IDomainEventHandler<UserAccountActivatedEvent>
    {
        ...
        private readonly ILogger<LogUserAccountActivatedAuditHandler> _logger;

        public LogUserAccountActivatedAuditHandler(
            IAuditLogService auditLogService,
            IUserRepository userRepository,
            ILogger<LogUserAccountActivatedAuditHandler> logger)
        {
        ...
        public async Task HandleAsync(UserAccountActivatedEvent @event, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "Recording audit log for User account activation. (User: {UserName}, Method: {Method})",
                    userName, @event.ActivationMethod);

                // 2. 감사 로그 요청 DTO 생성
                var details = new Dictionary<string, string?>
                {
                    { "ActivationMethod", @event.ActivationMethod },
                    { "ActivatedAt", @event.ActivatedAt.ToString("o") }
                };
                ...
                var auditRequest = new CreateAuditLogRequest
                {
                    ...
                    Success = true,
                    Severity = AuditEventSeverity.High, // 계정 활성화는 중요한 보안/상태 이벤트
                ...
                // TriggeredBy가 ConnectedId나 UserId일 수 있으므로, BaseEvent의 TriggeredBy를 사용합니다.
                // (일반적으로 관리자가 활성화했다면 TriggeredBy에 ConnectedId가 있을 것입니다.)
                ServiceResult<AuditLogResponse> result = await _auditLogService.CreateAsync(
                    auditRequest,
                    @event.TriggeredBy // BaseEvent의 TriggeredBy를 행위자로 사용
                );
                ...
                {
                    _logger.LogInformation(
                        "Audit log recorded successfully for account activation. (UserId: {UserId})",
                        @event.UserId);
                }
                else
                {
                    _logger.LogError(
                        "Failed to record audit log for account activation. Reason: {Error}",
                        result.ErrorMessage ?? "Unknown error");
                }
            }
            catch (OperationCanceledException)
            {
                _logger.LogWarning("Audit log recording for account activation was cancelled. (UserId: {UserId})", @event.UserId);
                throw;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex,
                    "Error in LogUserAccountActivatedAuditHandler. (UserId: {UserId})",
                    @event.UserId);
            }
        }
    }
